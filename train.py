    ...
    *,
    ...
    for epoch in range(config.n_epochs):
        ...
        if (epoch + 1) % config.train_output_every_n == 0:
            print(
                f'Epoch {epoch + 1}{f" ({(time.time()-start):0.2f}s)" if config.time_epochs else ""} -- Train Loss: {loss:0.5f}'
            )
        if (epoch + 1) % config.validate_every_n == 0 or config.final_run:
            if config.prthreshold > 0:
                prthreshold = config.prthreshold
            else:
                prthreshold = pick_threshold(
                    model,
                    train_loader,
                    config.undersample_ratio,
                    device,
                    verbose=config.verbose,
                    feature_count=feature_count,
                )
            acc, precision, recall, f1 = validate(
                model,
                test_loader,
                device,
                verbose=config.verbose,
...
def validate(
    ...
    if verbose:
        print(
            f"Mean output at 0: {(sum(preds_0) / len(preds_0)).item():0.5f} at 1: {(sum(preds_1) / len(preds_1)).item():0.5f}"
        )
...
def pick_threshold(
    model, dataloader, undersample_ratio, device, verbose=True, feature_count=13
):
    all_ys = []
    all_preds = []
    for batch in dataloader:
...
def create_transformer(config):
    if config.model == "AnomalyTransformer":
        ...
        return TransformerTimeSeries(
            config.feature_size, 1, config.n_head, config.n_layers, config.dropout
        ).to(device)
    elif config.model == "AnomalyTransfomerIntermediate":
        return AnomalyTransfomerIntermediate(
            config.segment_length,
            config.feature_size,
            config.n_layers,
            config.lambda_,
        ...
        return AnomalyTransfomerBasic(
            config.segment_length, config.feature_size, config.n_layers, device
        ).to(device)
...
def parse_args():
    ...
    args.add_argument(
        "--cell_norm", type=bool, default=False
    )  # bools are weird with argparse. deal with this later
...
if __name__ == "__main__":
    ...
    config.n_feats = (
        data.shape[-1] - 1 if config.feature_size == -1 else config.feature_size
    )
    ...
    for model_index, model_creator in enumerate(models):
        ...
        sample_model = model_creator(
            config
        )  # used only for debug output in the line below (and a similar line after all folds)
        print(
            f"Model {type(sample_model)} using {count_parameters(sample_model)} parameters:"
        )
        if config.kfolds > 1:
            ...
            for fold_i, (train_indices, test_indices) in enumerate(kf.split(data)):
                ...
                test_loader = create_loader(
                    test_data, batch_size=config.batch_size, drop_last=False
                )
                if (
                    config.model == "AnomalyTransfomerIntermediate"
                    or config.model == "AnomalyTransformer"
                ):
